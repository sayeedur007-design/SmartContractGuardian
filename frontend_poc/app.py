...
def start_analysis():
    ...
    model_config = ModelConfig(
        context_model=data.get('context_model', 'ollama'),
...
def analyze_thread(job_id, contract_path, model_config, auto_run_config, use_rag=True):
    try:
        ...
        if job.get('contracts_dir') and os.path.isdir(job['contracts_dir']):
            ...
            # Project context also supports a single uploaded Solidity file.  Passing
            # the file itself keeps the analysis isolated instead of scanning uploads/.
            contracts_dir = contract_path
            contract_info["contracts_dir"] = contracts_dir
    ...
    def analyze_contract(self, contract_info, auto_run_config=None):
        ...
        if "contracts_dir" in contract_info and contract_info["contracts_dir"]:
            ...
            insight_details = {
                ...
                # Preserve the graph inputs generated by ProjectContextLLMAgent.  The
                # frontend cannot derive inheritance/import relationships from filenames.
                'contract_details': project_context_results.get('contract_details', []),
                'mermaid_diagram': project_context_results.get('mermaid_diagram', ''),
                'stats': {
            ...
            # Keep the exact UI payload with the final analysis result as well as
            # emitting it live.  A client can otherwise miss this Socket.IO event
            # while reconnecting and has no way to recover the contract diagram.
            contract_info["project_context_ui"] = insight_details
        ...
        if contract_info.get("project_context_ui"):
            result["project_context"] = contract_info["project_context_ui"]
